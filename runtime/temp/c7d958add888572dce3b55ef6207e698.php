<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:87:"/www/wwwroot/vip.guochamall.com/public/../application/tmvip/view/user/wallet_chart.html";i:1525228938;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<script>
    ...
    function getDate2_1(curPage, time1, time2) {
        ...
        $.ajax({
            url: "<?php echo URL('tmvip/User/wallet_chart'); ?>",
            type: "post",
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.list.length; i++) {
                    for (var j = 0; j < data['list'][i].length; j++) {
                        ...
                        var grade = data['list'][i][j]['wallet_add'][j];//释放
                        var cz = data['list'][i][j]['wallet_out'][j];//茶券
                        var cz1 = data['list'][i][j]['wallet_use'][j];//茶点
                        names += ",'" + name + "'";
                        grades += "," + grade + "";
                        place += "," + cz + "";
                        place1 += "," + cz1 + "";
    ...
    function table1(datas){
        var str_table = ""
        for(i = 0;i< datas.length;i++){
             str_table += "<tr>";

                str_table += "<td>"+datas[i]['user_name']+"</td>";
                str_table += "<td>"+datas[i]['sum_inte']+"</td>";
                str_table += "<td>"+datas[i]['surplus_inte']+"</td>";
                str_table += "<td>"+datas[i]['introduce']+"</td>";
                str_table += "<td>"+getLocalTime(datas[i]['addtime'])+"</td>";

            str_table +="</tr>";
        }
        //console.log(str_table)
        $('#table1 >tbody').html(str_table)
    }
            ...
            success: function (data) {
                ...
                for (var i = curPage; i < count; i++) {
                    ...
                    str += "<tr>"+
                        ...
                        "<td>"+data.list1[i]['sum_inte']+"</td>"+
                         "<td>"+data.list1[i]['surplus_inte']+"</td>"+
                         "<td>"+data.list1[i]['introduce']+"</td>"+
