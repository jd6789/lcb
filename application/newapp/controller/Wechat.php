...
class Wechat
{
    ...
    public function __construct($options,$redis=null)
    {
...
//        $redis->select(4);
    ...
    {
//		log::record('receive____'.json_encode($this));
    ...
    public function getRevType() {
        log::record('$this->_receive'.json_encode($this->_receive));
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function setCache($cachename,$value,$expired){
        ...
        if(is_null($this->redisCache)){
            ...
            return $this->redisCache->set($cachename,$value,$expired);
        }
    }
...
//            return S($cachename);
//        }else{
//            return $this->redisCache->get($cachename);
//        }
//    }
    ...
    protected function removeCache($cachename){
        ...
        if(is_null($this->redisCache)){
            ...
            return $this->redisCache->del($cachename);
        }
    }
     ...
     * author:cc
     */
    public function checkAuth($appid='',$appsecret='',$token='', $force = false){
        /* $redis_config = C('REDIS');
         $redis = new \Redis();
         $redis->connect($redis_config['host'], $redis_config['port']);
         $redis->auth($redis_config['auth']);
         $redis->select(12);
         $result = json_decode($this->http_get(self::API_URL_PREFIX.self::AUTH_URL.'appid='.$this->appid.'&secret='.$this->appsecret),true);
         log::record('checkauth+++++++++'.json_encode($result));*/
        if (!$appid || !$appsecret) {
            $appid = $this->appid;
...
//        $cacheAccessToken = $this->getCache($authname);
//        if ($cacheAccessToken && $force == false)  {
//            $this->access_token = $cacheAccessToken;
...
//        }
    ...
    public function getWxImg($media_id){
        ...
        //原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
        //如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
        $url_prefix = self::API_URL_PREFIX;
        $result = $this->WxHttpGet($url_prefix.self::MEDIA_GET_URL.'access_token='.$this->access_token.'&media_id='.$media_id);
        $re = json_decode($result['body'],true);
        if ($re['errcode'] == '40001'){
            $this->checkAuth();
            $result = $this->WxHttpGet($url_prefix.self::MEDIA_GET_URL.'access_token='.$this->access_token.'&media_id='.$media_id);
        }
    ...
    private function WxHttpGet($url) {
        ...
        if (curl_getinfo($curl, CURLINFO_HTTP_CODE) == '200') {
            ...
            if(isset($header)){
                if(strpos($header,"Content-disposition")){
                    $positionStart = strpos($header,"filename");
//					log::record($positionStart);
                    $positionEnd = strpos($header,"Date");
//					log::record($positionEnd);
                    $filename = str_replace("\"","",trim(substr($header,$positionStart+strlen("filename")+1,$positionEnd-($positionStart+strlen("filename")+1))));
                }
            }
...
//			log::record($body);
        ...
        curl_close($curl);
        return ['filename' => isset($filename) ? $filename : '','body' => isset($body) ? $body : ''];
    }
    ...
    public function sendCustomMessage($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        foreach($data as $k=>$v) {
            $result = $this->http_post(self::API_URL_PREFIX . self::CUSTOM_SEND_URL . 'access_token=' . $this->access_token, self::json_encode($v));
        }
        if ($result)
        {
            $json = json_decode($result,true);
...
//		\Think\log::record('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&'.self::json_encode($data).'***********************************************************');
        ...
        \Think\log::record('********************************************************'.self::json_encode($result).'@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@');
        //print_r($result);
        if ($result['errcode'] == '40001'){
            $token = $this->checkAuth();
            $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_SEND_URL.'access_token='.$token,self::json_encode($data));
        }
        if($result){
