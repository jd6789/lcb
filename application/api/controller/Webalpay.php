...
class Webalpay extends Controller{
    ...
    private function config(){
        $data=Db::name('alpay_config')->find();
        return $data;
    }
    //移动端支付宝支付   下单购买以及再次购买，不包括升级购买
     /**
     * @param  $out_trade_no            需要支付的订单号
     *@param  $total_amount             需要支付的金额，需要精确到分
     *@param  $subject                  需要支付的订单的简单说明
     *@param  $body                     需要支付的订单的商品的说明
     *@param  $setReturnUrl             支付成功后的同步回调地址
     *@param  $payRequestBuilder        支付成功后的异步回调地址，所有的操作都是在这个链接里面实现，不带参数
     */
...
//        vendor("webalpay.wappay.buildermodel.AlipayTradeWapPayContentBuilder.php");
//        vendor(".webalpay.wappay.buildermodel.AlipayTradeWapPayContentBuilder.php");
