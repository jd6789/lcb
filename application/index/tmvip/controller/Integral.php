...
class Integral{
    ...
    public function everyInte(){
        ...
        $time = date('Y-m-d');
        //获取员工号
//        $gc_user = D('user')->where("username like 'xc%' or username like 'guocha%'")->select();
//        Db::connect('db_config1')->table('jieyu_user')->where('user_id',$info['id'])->order('id desc')->limit(1)->setField('is_return',' 0');
//        foreach($gc_user as $v_gc){
//            $rule['last_time'] = $time;
//            $rule['erevy_back_rate'] = 0;
//            $rule['grow_rate'] = 0;
//            D('Integral')->where('user_id = '.$v_gc['id'])->save($rule);
//        }

        //获取未返完的积分信息
        //$data = M('Integral')->where('is_return = 1')->select();
        $data = Db::table("tea_integral")->where('is_return ', 1)->select();
        foreach($data as $k=>$v){
            // 如果最后释放时间戳小于当前日前时间戳
