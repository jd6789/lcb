...
class Money extends Co{
    ...
    public function recharge(){
        ...
        $nopayorder=Db::table('tea_money')
            ...
            ->find();
        if($nopayorder) {
            $this->error('您有尚未支付的钱包充值订单，请支付或者取消','shareholder/inte_info');
        }
        return $this->fetch();
    }
    ...
    public function buyMoney(){
        $user_id=intval(session('user_id'));
        //首先判断用户是否被冻结
        $user_con=new \app\appmobile\controller\User();
    ...
    public function confirm($id){
        ...
        if($data){
            ...
            $this->error('数据非法，请重新下单','index/index');
        }
    }
