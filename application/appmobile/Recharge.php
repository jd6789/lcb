...
class Recharge extends Common {
    ...
    public function test(){
        ...
        $res=$con->asignManage(91,'2271524743081697168','2018042621001004350200332079');
        dump($res);die;
    }
    //显示所有的理茶宝信息
    ...
    //判断用户是否购买权限
    public function allowBuy($user_id=121,$recharge_money=500000){
    ...
    public function addAgainBuyManage(){
        ...
        //获取用户升级购买时参数
        $recharge_id = intval(input('post.recharge_id'));
        $rec_money = floatval(input('post.rec_money'));
        $teares=0;
        //在个人中心实现追加购买
        //①首先判断用户购买的金额是否大于最后一次购买的金额
        $actioninfo = Db::table('tea_user_recharge')
    ...
    public function allowAddBuyManage(){
        ...
        $user_id=session('user_id');
        $model=Db::table('tea_recharge_cart');
        $info = $model->where("user_id='$user_id' AND pay_status=0")->select();
        if($info){
...
//            $this->success('支付成功','appmobile/user/index');
    ...
    //支付时选择钱包金额
    private function payMoney($recharge_id,$money){
        ...
        if(floatval($rechargeInfo['recharge_money'])!=floatval($money)){
            ...
            if(intval($rechargeInfo['is_againbuy'])==0){
                //购买
                $controller->buyUpdate($user_id,$rechargeInfo['recharge_num'],00000000000000000000);
                $data['status']=1;
                $data['msg']='ok';
                return $data;
            }else{
                //升级
                $controller->asignManage($user_id,$rechargeInfo['recharge_num'],00000000000000000000);
                $data['status']=1;
                $data['msg']='ok';
                return $data;
            }
    ...
    public function allowproduct(){
        $user_id=session('user_id');
        $actioninfo = Db::table('tea_user_recharge')
            ->where("user_id=$user_id AND is_return=1 AND is_active= 1")
            ->order('rec_addtime desc')
            ->limit('1')->find();
        //dump($actioninfo);
        //判断用户是否能够升级
